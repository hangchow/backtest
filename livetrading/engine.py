...
class LiveTradingEngine:
    ...
    def on_account(self, account_id: str, snapshot: AccountSnapshot) -> None:
        ...
        with self._lock:
            ...
            self._account_state_store.sync_active_codes(account_id, self._current_config.all_codes())
            self._account_state_store.reconcile_from_actual(account_id, self._current_config.all_codes())
    ...
    def on_order_update(self, account_id: str, update: OrderUpdate) -> None:
        ...
        with self._lock:
            if self._current_config is None:
                return
            trade_account_map = self._current_config.trade_account_map()
        ...
        with self._lock:
            ...
            for code in config.all_codes():
                ...
                if price is not None and price > 0:
                    prices[code] = price
            accounts = config.trade_accounts
            pool_codes = config.stock_pool.codes
            account_contexts = [
                (
                    account,
                    self._account_state_store.ensure(account.account_id),
                    self._trade_account_clients.get(account.account_id),
                )
                for account in accounts
            ]

        for account, state, client in account_contexts:
