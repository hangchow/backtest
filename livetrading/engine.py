...
from .execution import DryRunRebalanceExecutor, TradeAccountState
...
class LiveTradingEngine:
    def __init__(
        ...
        *,
        ...
        self._dry_run_executor = DryRunRebalanceExecutor(self._logger)
    ...
    def _execute_portfolio_rebalance_dry_run(self, decision: PortfolioRebalanceDecision) -> None:
        """收集参考价并委托 DryRunRebalanceExecutor 执行调仓。"""
        with self._lock:
            ...
            self._dry_run_executor.execute_portfolio_rebalance(
                decision=decision,
                accounts=config.trade_accounts,
                account_states=self._account_states,
