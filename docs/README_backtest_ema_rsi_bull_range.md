...
```bash
./.venv/bin/python -m backtest.backtest_ema_rsi_bull_range --codes HK.00700 --market HK --initial-cash 800000
```

如果你不想隔夜持仓：

```bash
./.venv/bin/python -m backtest.backtest_ema_rsi_bull_range --codes HK.00700 --market HK --initial-cash 800000 --flat-at-close
```
